...
size_t post_feedback(void *ptr, size_t size, size_t nmemb, void *stream)//接受回传测试 
{
	printf("\n+++++++++++++下面是回传结果:+++++++++++++\n");
    printf("%s", (char*)ptr);
}
...
int libcurl(const char *POSTURL,char *post) //libcurl
{
	...
	curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,post_feedback);//接受回传测试？ 
	/*设置参数结束*/ 
...
void Login_or_Logout(char *post)  //选择登入还是登出 
{
	char action[]="&action:";
	strcat(post,action);
...
void Input_and_Encrypt(char *post) //输入并加密函数 
{
	char username[20];
	char password[20];
	char acid[8]="&acid:";
...
{ 
	...
	printf("+++++++Made By CHN-STUDENT && MouYouLing V0.0.5+++++++\n\n"); 
	Input_and_Encrypt(post);
	Login_or_Logout(post);
